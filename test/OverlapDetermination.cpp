...
void leftMostPlacementTest(
  const std::string &lineStr, const std::vector<int> &hintNumbers, const std::string &expectedStr) {
    Line line = Line(lineStr);
    HintList hintList = HintList(hintNumbers);
    Placement expected = Placement(expectedStr);

    Placement result = FindLeftMostPlacementAlgorithm::run(line, hintList);
...
TEST(Algorithm_LeftMostPlacement, ImpossibleCase) { leftMostPlacementTest("WW   ", {1, 2}, ""); }
...
void rightMostPlacementTest(
  const std::string &lineStr, const std::vector<int> &hintNumbers, const std::string &expectedStr) {
    ...
    Placement result = FindRightMostPlacementAlgorithm::run(line, hintList);
    EXPECT_EQ(result, expected);
}
...
TEST(Algorithm_RightMostPlacement, ImpossibleCase) { rightMostPlacementTest("   WW", {1, 2}, ""); }
...
TEST(Algorithm_OverlapDetermination, VerySimpleCase) {
    ...
    Line result = OverlapDeterminationAlgorithm::determineByOverlap(line, hintList);
    EXPECT_EQ(result, expected);
}

TEST(Algorithm_OverlapDetermination, EdgeWhiteCase) {
    Line line = Line(" W   ");
    HintList hintList = HintList({2});
