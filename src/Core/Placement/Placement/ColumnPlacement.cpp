...
ColumnPlacement::ColumnPlacement(std::string colorString) {
...
ColumnPlacement::ColumnPlacement(HintNumber hintNumber) {
    for (int i = 0; i < hintNumber.value; i++) {
        placement.emplace_back(Cell(CellColor::Black));
    }
}

const std::vector<Cell> &ColumnPlacement::getPlacement() const { return placement; }

bool ColumnPlacement::operator==(const ColumnPlacement &other) const {
...
Cell ColumnPlacement::operator[](const RowIndex &index) const { return placement[index.value]; }

ColumnPlacement ColumnPlacement::operator+(const ColumnPlacement &other) const {
    ...
    return ColumnPlacement(newPlacement);
}

ColumnPlacement &ColumnPlacement::operator+=(const ColumnPlacement &other) {
    placement.insert(placement.end(), other.placement.begin(), other.placement.end());
    assert(!hasNone());
    return *this;
}

bool ColumnPlacement::hasNone() const {
...
const size_t ColumnPlacement::size() const { return placement.size(); }

const std::vector<RowIndex> ColumnPlacement::getHintIndex() const {
    std::vector<RowIndex> hintIndexList;
