...
Placement::Placement(std::vector<Cell> placement) : placement(placement) { assert(!hasNone()); }
...
const std::vector<Cell> &Placement::getPlacement() const { return placement; }

bool Placement::operator==(const Placement &other) const { return placement == other.placement; }

bool Placement::operator!=(const Placement &other) const { return !(placement == other.placement); }

Cell Placement::operator[](const CellIndex &index) const { return placement[index.getIndex()]; }
...
RowPlacement Placement::toRowPlacement() const { return RowPlacement(placement); }

ColumnPlacement Placement::toColumnPlacement() const { return ColumnPlacement(placement); }
