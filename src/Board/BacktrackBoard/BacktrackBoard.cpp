...
BacktrackBoard::BacktrackBoard(
    NonogramBoard nonogramBoard, RowPlacementCountList rowPlacementCountList,
...
NonogramBoard BacktrackBoard::getNonogramBoard() const { return nonogramBoard; }
...
Board BacktrackBoard::getBoard() const { return nonogramBoard.getBoard(); }
...
bool BacktrackBoard::isSolved() const { return nonogramBoard.isSolved(); }
...
void BacktrackBoard::applyRow(RowIndex rowIndex,
...
void BacktrackBoard::applyColumn(ColumnIndex columnIndex,
...
void BacktrackBoard::applyAssumption(const IAssumption &assumption) {
  undoStack.push_back(nonogramBoard.getBoard());
  assumption.applyTo(*this);
}

void BacktrackBoard::revert() {
  if (undoStack.empty())
    return;

  nonogramBoard.applyBoard(undoStack.back(), true);
  undoStack.pop_back();
}
...
void BacktrackBoard::setRowPlacementCount(RowIndex rowIndex,
                                          PlacementCount placementCount) {
  rowPlacementCountList[rowIndex] = placementCount;
}

void BacktrackBoard::setColumnPlacementCount(ColumnIndex columnIndex,
                                             PlacementCount placementCount) {
