...
ImFont
  *FontData::fonts[(FontData::maxFontSize - FontData::minFontSize) / FontData::fontSizeStep + 1] =
    {};
...
void FontData::initFontData() {
    ...
    for (int fontSize = minFontSize, index = 0; fontSize <= maxFontSize;
         fontSize += fontSizeStep, index++) {
        fonts[index] = io.Fonts->AddFontFromFileTTF(
          "C:\\Windows\\Fonts\\consola.ttf", static_cast<float>(fontSize), &font_cfg,
          glyph_ranges_numbers);
