...
void TableRenderer::render(
  ...
  // const SharedHighlightIndexes &sharedHighlightIndexes
    ...
    const RowLength totalRowLength = columnHintLength + boardRowLength +
                                     columnPlacementCountLength + columnBacktrackStackLength;
    const ColumnLength totalColumnLength = rowHintLength + boardColumnLength +
                                           rowPlacementCountLength + rowBacktrackStackLength;
    ...
    ImGui::SetCursorPos(ImVec2(
      ImGui::GetCursorPosX() + (offset_x > 0 ? offset_x : 0),
    ...
    for (int r = 0; r < totalRowLength.getLength(); ++r) {
        for (int c = 0; c < totalColumnLength.getLength(); ++c) {
            ...
            ImVec2 p_min = ImVec2(
              table_start_screen_pos.x + c * cell_size, table_start_screen_pos.y + r * cell_size);
            ...
            if (isWindowHovered) {
                if (
                  mousePos.x >= p_min.x && mousePos.x < p_max.x && mousePos.y >= p_min.y &&
                  mousePos.y < p_max.y) {
                    isHovered = true;
                }
            }
            ...
            CellType cellType = determineCellType(
              rowIndex, columnIndex, columnHintLength, rowHintLength, boardRowLength,
              boardColumnLength, columnBacktrackStackLength, rowBacktrackStackLength);
            ...
            ImU32 color = getCellColorU32(
              rowIndex, columnIndex, columnHintLength, rowHintLength, board, cellType, isHovered);
            ...
            if (!label.empty()) {
                ImVec2 textSize = font->CalcTextSizeA(
                  font->LegacySize, FLT_MAX, 0.0f, label.c_str());
                ImVec2 textPos = ImVec2(
                  p_min.x + (cell_size - textSize.x) * 0.5f,
                  p_min.y + (cell_size - textSize.y) * 0.5f);

                draw_list->AddText(
                  font, font->LegacySize, textPos, IM_COL32(50, 50, 50, 255), label.c_str());
            }

            // 4. Draw Grid Lines
            drawGridLineDirect(
              draw_list, p_min, p_max, rowIndex, columnIndex, columnHintLength, rowHintLength,
    ...
    long long duration_data_copy = std::chrono::duration_cast<std::chrono::microseconds>(
                                     end_data_copy - start_data_copy)
                                     .count();
    long long duration_draw_loop = std::chrono::duration_cast<std::chrono::microseconds>(
                                     end_draw_loop - start_draw_loop)
                                     .count();
    ...
    ImGui::Text("1. Data Copy Time (Lock Wait + Copy): %lld us", duration_data_copy);
    ImGui::Text("2. Main Draw Loop Time (CPU Cycles): %lld us", duration_draw_loop);
    ImGui::Text(
      "3. Other (ImGui Overhead, Layout, etc.): %lld us",
...
// Style Stack)
ImU32 TableRenderer::getCellColorU32(
  RowIndex rowIndex,
  ColumnIndex columnIndex,
  RowLength columnHintLength,
  ColumnLength rowHintLength,
  ...
  bool isHovered) const {
    ...
    if (cellType == OUT_OF_BOARD) {
      ...
      cellType == ROW_PLACEMENT_COUNT || cellType == ROW_HINT || cellType == ROW_BACKTRACK_STACK) {
    ...
    if (
      ...
      cellType == COLUMN_BACKTRACK_STACK) {
        ...
        if (cellType == ROW_BACKTRACK_STACK) {
          ...
          //int backtrackDepth = nonogramBoard.getDepthAtRowIndex(backtrackStackIndex);
        ...
        if (cellType == COLUMN_BACKTRACK_STACK) {
          ColumnIndex backtrackStackIndex = columnIndex - rowHintLength;
            int backtrackDepth =
              nonogramBoard.getDepthAtColumnIndex(backtrackStackIndex);
          return std::to_string(backtrackDepth);
        }
          */
    }
...
void TableRenderer::drawGridLineDirect(
  ...
  ColumnLength boardColumnLength) const {
    ...
    draw_list->AddLine(
      ImVec2(p_max.x, p_min.y), ImVec2(p_max.x, p_max.y), IM_COL32(0, 0, 0, 255), columnThickness);
